...
async def check_inventory(product_name: str) -> str:
    ...
    if not matches:
        return f"No products matching '{product_name}'. Available: laptops, accessories, monitors."
    lines = [
...
@mcp.tool()
async def search_products(query: str = "", category: str = "", in_stock_only: bool = False) -> str:
...
async def get_order_status(order_id: str) -> str:
    ...
    if not order:
        return f"Order {order_id} not found. Please verify the ID (format: ORD-XXXXX)."
...
@mcp.resource("docs://refund-policy", name="Refund Policy", mime_type="text/markdown")
...
def analyze_order(order_id: str) -> str:
    """Analyse an order for potential issues or anomalies."""
    order = ORDERS.get(order_id.upper(), {"status": "unknown", "items": [], "total": 0})
    return (
        f"Analyse order {order_id}:\n{json.dumps(order, indent=2)}\n\n"
